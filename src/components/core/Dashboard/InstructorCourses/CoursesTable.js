              ...
              >
                <Td colSpan={1} className="flex flex-1 gap-x-4 p-3">
                  <img
                    ...
                    className="md:h-[148px] md:w-[220px] aspect-video rounded-lg object-cover cursor-pointer"
                    onClick={()=>navigate(`/courses/${course._id}`)}
                  />
                  <div className="flex flex-col gap-1 justify-between">
                    <p className="text-lg font-semibold text-richblack-5 mt-3  truncate tracking-wide">
                      {course.courseName}
                    </p>
                    ...
                    <ul style={{ listStyle: 'none', padding: 0 }} className="tracking-wider">
                      < li style={{ display: 'flex', alignItems: 'flex-start' }} className="text-xs text-richblack-300">
                          <span>{course.courseDescription.split('.')[0].trim()}.</span>
                       </li>
                         </ul>


                    <p className="text-[12px] text-white tracking-widest  lg:text-left text-center">
                      Created: {formatDate(course?.createdAt || course?.updatedAt)}
                    </p>
                    {course.status === COURSE_STATUS.DRAFT ? (
                      <p className="flex w-fit flex-row items-center gap-2 rounded-full bg-richblack-700 px-2 py-[2px] text-[12px] font-medium text-pink-100  tracking-wider">
                        <HiClock size={14} />
                        Drafted
                      </p>
                    ) : (
                      <p className="flex w-fit flex-row items-center gap-2 rounded-full bg-richblack-700 px-2 py-[2px] text-[12px] font-medium text-yellow-100  tracking-wider">
