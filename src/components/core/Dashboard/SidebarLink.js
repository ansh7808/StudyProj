...
const SidebarLink = ({ data }) => {
  ...
  return (
    <div>
      <Link
        to={data.path}
        className={`relative flex gap-x-2 items-center text-sm font-medium px-3 md:px-8 py-2 cursor-pointer 
      ...
      >
        ...
        <p className='hidden md:block tracking-wider'>{data.name}</p>
      </Link>
    </div>
  )
}
