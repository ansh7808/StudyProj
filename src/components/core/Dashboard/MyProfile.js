...
const MyProfile = () => {
  ...
  return (
    <div className='bg-richblack-900 text-white mx-0 md:mx-5' >
      ...
      <div className='flex items-center justify-between rounded-md border border-richblack-700 bg-richblack-800
       p-8 px-3 md:px-12'>
        {/* section 1 */}
        <div className='flex flex-row gap-x-2 md:gap-x-4 items-center' >
          <div>
            <img src={user?.image} alt={`profile-${user?.firstName}`} className='aspect-square w-[60px] md:w-[78px] 
        ...
        <IconBtn customClasses={"hidden md:block"} text={'Edit'} onClick={() => navigate('/dashboard/settings')} 
      ...
      <div className='md:hidden'>
       ...
       {/* section 2 */}
      <div className='flex flex-col gap-y-10 my-7 md:my-10 rounded-md border border-richblack-700 p-8 px-3
       md:px-12 bg-richblack-800'>
        <div className='flex items-center justify-between' >
          <h2 className='text-lg font-semibold text-richblack-5'>About</h2>
          <IconBtn text={'Edit'} customClasses="hidden md:block" onClick={() => navigate('/dashboard/settings')} 
          children={<RiEditBoxLine />} />
        </div>
      ...
      <div className='flex flex-col gap-y-5 md:gap-y-10 rounded-md border border-richblack-700 p-8 px-3 md:px-12 bg-richblack-800 '>
        ...
        <div className='flex flex-col md:flex-row gap-y-5'>
          ...
          <div className='w-full md:w-1/2'>
            ...
            >{user?.additionalDetails?.dateOfBirth ? formattedDate(user?.additionalDetails?.dateOfBirth) : 'Add Date of Birth'}</p>
          </div>
        </div>
      </div>
